...
- Export targets double as their freshness tags (`exports/statements.csv`, `entities.ftm.json`, `exports/documents.csv`, `exports/documents.{origin}.csv`, `exports/statistics.json`, `index.json`). The `all` sweep targets `operations/export/last_run` and stamps every artifact tag it wrote from one `ExitStack` of `touch()` calls – so a crash part-way stamps nothing, and a later single-kind export still sees itself fresh
