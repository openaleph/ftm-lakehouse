...
│   ├── maintenance.py   # `maintenance` group (optimize, unlock)
...
2. **Writing (direct bulk)**: Entities → `EntityBuffer` (in-memory, shard-sorted) → `repo.write_statements` (packs per shard, appends through the one `write_batches` loop) → Parquet Store. With `--unsafe` on the CLI import commands: payload dicts → `logic/entities/explode.py` (packed row dicts, no FtM object construction; parity-tested against the safe path incl. statement ids and namespace stripping) → `RowBuffer` (flushes a `SHARDED_SCHEMA` table) → `repo.write_batches` → Parquet Store
...
- `statements/last_updated` – Parquet store was updated
- `statements/last_optimized` – Optimize ran (merge + compact + vacuum)
...
- Sub-typer groups: `maintenance` (+ top-level `configure` / `make` / `export` / `crawl` shortcuts), `entities`, `statements`, `archive`, `zfs`
