...
def _test_archive(archive: DatasetArchive, crawl_uri: Uri):
...
def test_archive_local(tmp_path, fixtures_path):
    archive = DatasetArchive("test", tmp_path)
    assert _test_archive(archive, fixtures_path / "src")


@mock_aws
def test_archive_s3_dataset(fixtures_path, moto_server):
    moto_server.create_bucket(Bucket="lakehouse")
    archive = DatasetArchive("test", "s3://lakehouse/test")
    assert _test_archive(archive, fixtures_path / "src")
